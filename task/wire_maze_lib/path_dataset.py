...
def rotate_maze_and_path_90(maze, path, num_times=1):
    """Rotate maze and path 90 degrees."""
    maze = np.rot90(maze, k=num_times)
...
def orient_maze_and_path(maze, path):
    ...
    Args:
        maze: Binary array of size [2 * height - 1, 2 * width - 1].
    ...
    if start_orig[0] == 0:
        ...
        maze, path = rotate_maze_and_path_90(maze, path)
        return orient_maze_and_path(maze, path)
...
def generate_path_dataset(path_generator, num_samples, num_tries):
    ...
    for i in range(num_tries):
        ...
        maze, path = orient_maze_and_path(maze, path)
        ...
        if len(mazes[pair]) >= num_samples:
            ...
            flip_maze, flip_path = (
                flip_maze_and_path_left_right(maze, path))
            if not np.array_equal(maze, flip_maze):
                ...
                mazes[flip_pair].append(flip_stim)
