...
class PathGenerator():
    ...
    def _reset(self):
        ...
        self._add_tail_pos()
        self._update_maze(just_turned=False)
    ...
    def _update_maze(self, just_turned):
        ...
        if self._maze_array[self._tail_position[0], self._tail_position[1]] != 0:
            ...
            if just_turned:
                ...
                if behind_in_maze:
                    if self._maze_array[behind[0], behind[1]] != 0:
                        return False
                    else:
    ...
    def _step(self):
        ...
        if self._since_turn >= self._min_segment_length:
            if np.random.rand() < self._turn_prob:
                ...
                self._tail_position += 2 * self._tail_direction
        else:
            self._tail_position += 2 * self._tail_direction
        self._add_tail_pos()
        just_turned = self._since_turn == 0
        valid_update = self._update_maze(just_turned)
        return valid_update
